...
const renderExpr = async (expr: Expression, ctx: RenderContext) : Promise<string> => {
  ...
  return `(${await render(expr.left, ctx)} ${renderOperator(expr.operator, ctx)} ${await render(expr.right, ctx)})`;
};
...
const render = async (tree:any, ctx: RenderContext): Promise<string> => {
  if (tree === undefined) return '';
  else if (isTerm(tree)) return await renderTerm(tree, ctx);
  else if (isPhrase(tree)) return await renderPhrase(tree, ctx);
  else if (isRange(tree)) return renderRange(tree, ctx);
  else if (hasOperator(tree)) return await renderExpr(tree, ctx); // binary or unary expression
  else if (hasLeft(tree)) return await render(tree.left, ctx);    // an operator-less (single term) expression
 ...
 *  
 */
export const lq2xq = async (lq: string, option? : RenderOptions) : Promise<string> => {
  ...
  if (hasPrefix(tree)) {
    ...
    if (!hasFieldInExr) {
      let plusExprs = await Promise.all(_.map(pluses, (p) => render(p, new RenderContext(option, hasFieldInExr))));
      let minusExprs = await Promise.all(_.map(minuses, (p) => render(p, new RenderContext(option, hasFieldInExr))));
    ...
    result += await render(tree, new RenderContext(option, hasFieldInExr));
  }
  return result;
};