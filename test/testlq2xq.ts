...
exports.single = (test: nodeunit.Test) => {
  (async function body() {
    ...
    test.strictEqual(await lq2xq("t1"), expected);
...
exports.or = (test: nodeunit.Test) => {
  (async function body() {
    ...
    test.strictEqual(await lq2xq("t1 t2"), expected);
    test.strictEqual(await lq2xq("t1 OR t2"), expected);
    test.strictEqual(await lq2xq("t1 || t2"), expected);
  })().catch(test.ifError).then(test.done);
}
exports.and = (test: nodeunit.Test) => {
  (async function body() {
    ...
    test.strictEqual(await lq2xq("t1 AND t2"), expected);
    test.strictEqual(await lq2xq("t1 && t2"), expected);
  })().catch(test.ifError).then(test.done);
}
...
exports.andor = (test: nodeunit.Test) => {
  (async function body() {
    ...
    test.strictEqual(await lq2xq(expr), expected);
    test.strictEqual(await lq2xq("t1 AND (t2 || (t3 && t4))"), expected);
  })().catch(test.ifError).then(test.done);
}

exports.andorgroup = (test: nodeunit.Test) => {
  (async function body() {
    test.strictEqual(await lq2xq("(t1 AND t2) || t3 && t4"), 
...
exports.phraseSingle = (test: nodeunit.Test) => {
  (async function body() {
    test.strictEqual(await lq2xq(`"i am"`), `. contains text ("i am")`);
  })().catch(test.ifError).then(test.done);
}
...
exports.range = (test: nodeunit.Test) => {
  (async function body() {
    ...
    test.strictEqual(await lq2xq(`title:{aida TO carmen}`), `(title>"aida" AND title<"carmen")`);
    test.strictEqual(await lq2xq(`title:[aida TO carmen] token`),
...
exports.wildcard = (test: nodeunit.Test) => {
  (async function body() {
    test.strictEqual(await lq2xq(`test*`), `. contains text ("test.*" using wildcards)`);
    test.strictEqual(await lq2xq(`title:test?d`), `(title contains text "test.d" using wildcards)`);
  })().catch(test.ifError).then(test.done);
}
exports.proximity = (test: nodeunit.Test) => {
  (async function body() {
    test.strictEqual(await lq2xq(`"i am"~2`), `. contains text ("i am" distance at most 2 words)`);
    test.strictEqual(await lq2xq(`title:"i am"~2`), `(title contains text "i am" distance at most 2 words)`);
  })().catch(test.ifError).then(test.done);
}
...
exports.fuzzy = (test: nodeunit.Test) => {
  (async function body() {
    ...
    test.strictEqual(await lq2xq(`token~0.9`, {similarityConverter: (similarity)=>{
...
exports.modifiers = (test: nodeunit.Test) => {
  (async function body() {
    test.strictEqual(await lq2xq(`t1 +t2 -t3 +t4`), `. contains text ("t2") ftand ("t4") ftand ftnot ("t3")`);
  })().catch(test.ifError).then(test.done);
}
...
exports.fieldGroup = (test: nodeunit.Test) => {
  (async function body() {
    test.strictEqual(
        await lq2xq("title:(+t2) title:(-t3)"),
        '(title contains text "t2") AND not(title contains text "t3")'
    )
    test.strictEqual(
        await lq2xq("title:(t1 +t2 -t3)"),
        '(title contains text "t2") AND not(title contains text "t3")'
    )
    test.strictEqual(
        await lq2xq("title:(+t2) title:(-t3) +ta -tb"),
...
exports.xpathField =(test: nodeunit.Test) => {
  (async function body() {
    test.strictEqual(
        await lq2xq("title:(+t2) //object_name:-name"),
...
exports.expander =(test: nodeunit.Test) => {
  (async function body() {
    test.strictEqual(
        await lq2xq("abc AND efg", {termPhraseExpander: (term, field, isPhrase)=>Promise.resolve([term, 'new'+term])}),
        '. contains text (("abc" ftor "newabc") ftand ("efg" ftor "newefg"))'
    )

    test.strictEqual(
        await lq2xq("abc AND title:efg", {
